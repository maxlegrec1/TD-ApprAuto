...
from utils.data import COLUMNS, TARGET_FEATURES
...
data = pd.read_csv(
    "welddb/welddb.data", delim_whitespace=True, header=None, names=COLUMNS
)
...
def weld_type_to_rename(columns: list[str]):
    ...
    no_data = (
        {}
    )  # weld_type:list[feature] where feature is never given for weld_type instance
    #
    missing_values_by_weld_type = (
        pd.DataFrame()
    )  # with (index,column) we can see the probability of "index missing" when column is the weld_type
